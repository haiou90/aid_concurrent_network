...
* 基本概念
  ...
  * 数据 ： 能够输入到计算机中并被识别处理的信息集合。 <!--能够输入的内容：天气、人，借助数学-->
  * 大数据：是指无法在一定时间范围内用一定工具进行捕捉、管理和处理的数据集合，是海量、高增长率和多样化的信息资产。  <!--特点1：海量 特点2：变化-增长、多样化、不确定性-->

* 数据存储阶段

  * 人工管理阶段：人为管理，没有固定的格式和存储方法，容易混乱。<!--原始-->

  *  文件管理阶段 ：数据可以长期保存，存储数据量大，使用简单。<!--比较简单2天-->

  * 数据库管理阶段：高效，可以存储更大量数据，便于管理，更加专业。<!--比较专业5天-->
...
* 字节串类型
  ...
  * 概念 ： 在python3中引入了字节串的概念，与str不同，字节串以字节序列值表达数据，更方便用来处理二进程数据。<!--用来表达二进制数据的一种数据类型-->
...
| 打开模式 | 效果                                                         |
| -------- | ------------------------------------------------------------ |
| r        | 以读方式打开，文件必须存在  <!--read-->                      |
| w        | 以写方式打开，文件不存在则创建，存在**清空**原有内容  <!--write，兼具创建功能--> |
| a        | 以追加模式打开，文件不存在则创建，存在则**继续**进行写操作 <!--append,也是写--> |
| r+       | 以读写模式打开 文件必须存在  <!--读写，从头开始写，向后覆盖，有可能覆盖半个汉字，出现乱码--> |
| w+       | 以读写模式打开文件，不存在则创建，存在**清空**原有内容 <!--写读,清空原有--> |
| a+       | 追加并可读模式，文件不存在则创建，存在则**继续**进行写操作 <!--追加，偏移在末尾，不偏移读到内容为空--> |
| rb       | 以二进制读模式打开 同r                                       |
| wb       | 以二进制写模式打开 同w                                       |
| ab       | 以二进制追加模式打开 同a                                     |
| rb+      | 以二进制读写模式打开 同r+                                    |
...
* 定义
  ...
  > 2. a方式打开，文件偏移量在文件结尾位置  <!--例外：a+读的时候从结尾读,a+写的时候强制为从结尾开始，seek不起作用-->             
...
* 普通字符      <!--匹配自身-->
 ...
 匹配规则：每个普通字符匹配其对应的字符 <!--表示一个规则的描述，是整体不能单独拆出来-->
...
匹配规则: 匹配 | 两侧任意的正则表达式即可   <!--竖线两侧不能加空格-->
...
元字符：`.`        <!--常用重要，通常最多用两个点，超过两个采用其他替代方法-->
...
说明: 普通字符指数字，字母，下划线，汉字。 <!--python比其他语言匹配多了汉字等普通UTF-8字符，标点符号不属于普通字符-->
...
匹配规则：` \b` 表示单词边界，`\B` 表示非单词边界  <!--#(),等都是边界-->

说明：单词边界指数字字母(汉字)下划线与其他字符的交界位置。 <!--\b退格是转义字符需加r，为了预防出现转义字符的问题，不管有没有转义字符都要加r: \n-->
...
* 基本概念
* 数据库 ： 按照数据一定结构，存储管理数据的仓库。数据库是在**<u>数据库管理系统管理和控制</u>**下，在一定**介质**上的数据集合。   <!--大多数存储硬盘，个别在内存中存储，开辟空间-->
  
* 数据库管理系统 ：管理数据库的软件，用于建立和维护数据库。 <!--学会使用-->
...
* 数据库分类和常见数据库
  ...
  * 开源和非开源	
  ...
  > 非开源：Oracle、DB2、SQL_Server   <!--oracle IBM microsoft-->
...
>2. 库名区分字母大小写
>3. 不要使用特殊字符和mysql关键字         <!--`stu`为防止与关键字重名，用``反撇符号引起来-->
...
4. 查看当前所在库   
>select database();  <!--注意加括号-->

5. 删除库

>drop database 库名;   <!--尽量不要删 有风险-->
...
* 数字类型：
  ...
  * 浮点类型：FLOAT，DOUBLE，DECIMAL <!--float-->  float精确38位,  double精确308位
  * 比特值类型：BIT <!--bit-->
  
  <!--常用：tinyint int float bit-->
...
> 3. INT(2)  2代表显示宽度，但是范围仍然是int本身的范围(0,4294967295)<!--int数据类型为大整型，有符号存储范围为-2**31 ~ +2**31 -- 1，int(2)括号中的2仅仅为显示宽度，和存储数据的位数无关，而字符类型的宽度则为实际存储数据的宽度，二者在本质上是不同的-->

----------------------------------

* 字符串类型：
  * 普通字符串： CHAR，VARCHAR
  * 存储文本： text <!--文本也是字符串，不常用的存储在text,不能设置默认值-->
  * 存储二进制数据： BLOB <!--存储二进制数据-->
  * 存储选项型数据：ENUM，SET
  
  <!--char varchar text enum set常用-->

![](./img/字符串.png)

> 注意：
>
> 1. char：定长，即指定存储字节数后，无论实际存储了多少字节数据，最终都占指定的字节大小。**默认**只能存1字节数据。存取效率高。 <!--char(50),插入查找效率高-->
>
> 2. varchar：不定长，效率偏低 ，但是节省空间，实际占用空间根据实际存储数据大小而定。**必须要指定存储大小** varchar(50)。<!--数据库空间更重要，速度用其他方式解决，程序员常用-->
>
> 3. enum用来存储给出的多个值中的一个值,即单选，enum('A','B','C')
>
> 4. set用来存储给出的多个值中一个或多个值，即多选，set('A','B','C')  --->("A,B")
>
>    <!--MySQL数据类型中的枚举类型分为单选enum()和多选set()，在set()类型的字段插入记录有两个要求：1、字段值必须在给定值的范围内选择；2、选择多个值时用一对引号把所有值引起来，不同值之间用逗号隔开-->
...
* 字段约束
  * 如果你想设置数字为无符号则加上 unsigned <!--必须跟在数值类型后面-->
...
<!--NULL为空值，””为空字符串，匹配空值（NULL）必须用 is 或者 is not，而匹配空字符串（””）则用 = 或者 !=-->
...
<!--MySQL数据类型中的日期时间类型有两个函数，datetime()和timestamp()，datetime()在不给值时默认返回值为NULL，而timestamp()在不给值时会默认返回系统的当前时间。-->
...
<!--能用时间存储的尽量用时间存储，时间类型空间小，可以校验不符合要求时报错-->
...
* as 用法
  ...
  在sql语句中as用于给字段或者表重命名 <!--只针对这条语句有效-->
...
聚合操作指的是在数据查找基础上对数据的**进一步整理筛选行为**，实际上聚合操作也属于数据的查询筛选范围。  <!--更高级的数据整理筛选行为-->

#### 3.8.1 聚合函数 

<!--对查询结果直接进行函数操作,也可以对聚合分组后结果进行函数操作 --> 

 <!--where子句中不能使用聚合函数,聚合函数可以用在select having后面,因为where在groupby语句前执行-->
...
eg3 : 蜀国英雄中攻击值大于200的英雄的数量   <!--count不统计null空值，尽量用*统计表记录数-->
...
<!--count(name) name 不行-->    <!--count(name) max(name) 可以-->
...
e.g. :  对多个字段创建索引，此时多个字段都相同时为一组  <!--组数为age * sex-->
```mysql
...
<!--使用groupby分组查询时,select后只能写分组字段,不能写其他字段,但是可以写聚合函数,此时聚合函数相当于对分组后的 每个组 进行单独统计-->
...
> 2. having语句存在弥补了where关键字不能与聚合函数联合使用的不足,where只能操作表中实际存在的字段。 <!--只能将聚合函数重命名一个新名字a，把a看做一个字段在where中使用-->
...
<!--数据表按照一定的结构存储数据,内部结构要兼顾增删改查效率, 但建立的数据表可能90%操作是查询,所以需要提升查询效率,降低其他效率,因此可以通过建立索引解决这个问题-->

<!--比如经常查询name字段,可以根据name字段再建立一张索引表,索引表结构有利查询(索引表结构为树状结构或哈希结构),先在索引表查询,再根据查询结果到原表中提取,效率会提升几倍-->
...
* 在已有表中创建索引：  <!--添加普通索引和唯一索引, 索引名字不能重复,针对同一字段的索引重复创建会覆盖-->
...
- 主键索引添加 <!--通常很少添加主键,一般创建表时会设置-->

 ```sql
 alter table 表名 add primary key(id);
 ```

<!--alter table person add constraint dept_fk foreign key(dept_id) references dept(id);--> 创建外键
...
<!--alter table person drop foreign key dept_fk;--> #删除外键约束

<!--drop index dept_fk on person-->#同时删除索引名
...
* 约束 : 约束是一种限制，它通过对表的行或列的数据做出限制，来确保表的数据的完整性、唯一性
  * <!--外键约束: 对从表中关系字段的约束,约束关系字段不要超出主表范围-->
...
* 级联动作  <!--从表与主表建立起联系后，主表中数据变化(删除 修改)对从表的影响状态，从表(删除 修改)不影响主表-->
  * restrict(默认)  :  on delete restrict  on update restrict     <!--常用-->
    * 当主表删除记录时，如果从表中有相关联记录则不允许主表删除
    * 当主表更改主键字段值时，如果从表有相关记录则不允许更改
  * cascade ：数据级联更新  on delete cascade   on update cascade  <!--常用-->
...
当我们应对复杂的数据关系的时候，数据表的设计就显得尤为重要，认识数据之间的依赖关系是更加合理创建数据表关联性的前提。常见的数据关系如下： <!--一对多和多对多使用场景较多-->

- 一对一关系     平等关系<!--foreign key + unique-->
...
- 一对多关系    不平等关系  <!--主表是一个,从表是多个,外键字段和外键约束都在从表-->
...
>
> <!--id官方推荐整型,占用空间小,但不一定是整形型，如id不是整型则不能用auto increment,同时从表数据类型与主表一致,如都为字符串-->
...
> 也能对应A表中的多条记录
>
> 举例：一个运动员可以报多个项目，每个项目也会有多个运动员参加,这时为了表达多对多关系需要单独创建关系表。 <!--一个从表两个外键字段关联两个主表-->
...
![er](/Users/mac/Downloads/day09/er.png)
...
- 左连接  : 左表为主表，显示右表中与左表匹配的项 <!--显示左表全部内容以及右表与左表值匹配的内容-->  <!--官方希望把数据量大的表作为主表，基准表，左表放在前面，所以出现左右连接--> 
...
- 右连接 ：右表为主表，显示左表中与右表匹配的项  <!--显示右表全部内容以及左表与右表值匹配的内容--> <!--前面的是左表，后面的是右表-->
...
<!--查询结果做了一个快捷方式（映像，虚拟表），以一个二维表的方式呈现，不占用硬盘--><img src="/Users/mac/Desktop/day08 2/view.png" alt="view" style="zoom:50%;" />
...
* 视图表的增删改查操作
  ...
  视图的增删改查操作与一般表的操作相同，使用insert update delete select即可，但是原数据表的约束条件仍然对视图产生作用。 <!--多表联合视图很少进行insert，约束太多不容易插入-->
...
* 视图作用
  * 作用 <!--不是提高执行效率，而是提高个人工作效率-->

  1. 是对数据的一种重构，不影响原数据表的使用。

  2. 简化高频复杂操作的过程，就像一种对复杂操作的封装。  <!--场景1-->

  3. 提高安全性，可以给不同用户提供不同的视图。   <!--场景2：vip客户+普通客户提供不同数据-->
...
<!--函数：形成一定功能代码集合， 功能、参数、返回值-->

<!--对一定功能的sql语句封装-->

<!--函数：有返回值的函数，更加突出于要返回值得结果-->  #数据库优化要求尽量不用自定义函数

<!--存储过程：没有返回值得函数，更加突出于执行过程-->  #面试、工作中主要是存储过程

存储过程和函数是事先经过编译并存储在数据库中的一段sql语句集合，调用存储过程和函数可以简化应用开发工作，提高数据处理的效率。

<!--函数之间、存储过程之间在整个mysql里不能重名，但是函数与存储过程可以同名-->
...
* 设置变量
  * 用户变量方法：  set  @[变量名] = 值；使用时用@[变量名]。  <!--类似全局变量-->
  *  局部变量 ： 在函数内部设置  declare [变量名] [变量类型] ；局部变量可以使用set赋值或者使用into关键字。   <!--只有select能给变量赋值，增删改不可以-->
...
3. 终止事务，若begin之后使用commit提交事务或者使用rollback进行事务回滚。 <!--二选一-->
```mysql
...
> 数据库允许多个并发事务同时对其数据进行读写和修改的能力，而多个事务相互独立。隔离性可以防止多个事务并发执行时由于交叉执行而导致数据的不一致。  <!--mysql内部加锁实现-->
...
事务四大特性中的隔离性是在使用事务时最为需要注意的特性，因为隔离级别不同带来的操作现象也有区别  <!--严格程度不同-->

* 隔离级别

   - 读未提交：read uncommitted   <!--A的改动 B可同步看到 几乎不隔离-->
   
     > 事物A和事物B，事物A未提交的数据，事物B可以读取到
     > 这里读取到的数据叫做“脏数据”
     >     这种隔离级别最低，这种级别一般是在理论上存在，**数据库隔离级别一般都高于该级别**

   - 读已提交：read committed  <!--A提交了，B可以立即看到，因为变了，所以不可重复读-->
   ...
   - 可重复读：repeatable read<!--A修改提交了，B看不到修改，看到原来，所以可重复读-->
     ...
     >      MySQL默认级别
     >
     > 虽然可以达到可重复读取，但是会导致“幻像读”   <!--A改age，B改salary B看到两个都改了 -->
...
- 第一范式： 数据库表的**每一列都是不可分割的原子数据项，而不能是集合，数组，记录等组合**的数据项。简单来说要求数据库中的表示二维表，每个数据元素不可再分。

  例如： 在国内的话通常理解都是姓名是一个不可再拆分的单位，这时候就符合第一范式；但是在国外的话还要分为FIRST NAME和LAST NAME，这时候姓名这个字段就是还可以拆分为更小的单位的字段，就不符合第一范式了。

- 第二范式： 第二范式（2NF）要求数据库表中的每个实例或记录必须可以被唯一地区分，所有属性依赖于主属性。即选取一个能区分每个实体的属性或属性组，作为实体的唯一标识，每个属性都能被主属性筛选。其**实简单理解要设置一个区分各个记录的主键就好了**。

- 第三范式： 在第二范式的基础上属性不传递依赖，即每个属性不依赖其他非主属性。要求一个表中不包含已在其它表中包含的非主关键字信息。其实简单来说就是合理使用外键，使不同的表中不要有重复的字段就好了。<!--不依赖其他表的非主键而依赖主键，即合理的设置外键，关联主表的主键-->

<!--面试回答：遵循前三个范式-->

#### 3.14.2  MySQL存储引擎

* **定义**： mysql数据库管理系统中用来处理表的处理器    <!--数据库操作核心代码-->
...
- 如果很少被查询可以用 TIMESTAMP（时间戳实际是整形存储）  <!--此时优于datetime-->
...
- 尽量设置占用空间小的字段为主键   <!--常用int，索引占的空间也小-->
- 外键的设置用于保持数据完整性，但是会降低数据导入和操作效率，特别是高并发情况下，而且会增加维护成本   
- 虽然高并发下**不建议使用外键约束**，但是在表关联时建议在关联键上建立索引，以提高查找速度
...
- 尽量选择数据类型占空间少，在where ，group by，order by中出现的频率高的字段**建立索引**

- 尽量避免使用 select * ...;  用**具体字段**代替 * ,不要返回用不到的任何字段 

- 少使用like %查询，否则会全表扫描  <!--少使用模糊查询 正则查询-->
...
水平拆分 ： 减少每个表的数据量，通过**关键字**进行划分然后拆成多个表   <!--姓氏 区域-->
...
1. 表能根据实际需求复制数据
2. 复制表时不会把KEY属性复制过来  <!--索引 主键 外键不复制-->

**语法**

```mysql
create table 表名 select 查询命令;
e.g.    create table shu select * from sanguo where country="蜀";
```
...
1. 备份命令格式     <!--MySQL软件提供的，不是MySQL语句，要在liniux终端命令行执行-->
> mysqldump -u  用户名  -p  源库名  >  ~/stu.sql    <!--输入数据库密码123456-->
>
...
2.sudo vi mysqld.cnf  找到43行左右,加 # 注释 i ESC
...
7.ifconfig 查看IP地址 127.0.0.1
8.mysql -h 127.0.0.1 -u root -p
```

![数据库使用](/Users/mac/Desktop/day09 2/数据库使用.png)
...
3. 权限管理
   ...
   e.g.
   grant select,insert on stu.cls to 'vip'@'%' identified by '123' with grant option;
		with grant option 可以将自己权限转予新的用户
...
2. 创建游标对象(cur = db.cursor())    <!--游标对象负责调用执行SQL语句，操作数据，得到结果-->
...
```python
db.commit() 提交到数据库执行    #不是自动提交，必须写commit，建议执行多条语句 再commit
db.rollback() 回滚，用于当commit()出错是回复到原来的数据形态  #还受系统缓存影响如缓存满后自动写入
db.close() 关闭连接
```

#innodb  对于支持事务的引擎，按照事务缓存执行,占用内存较大 ，rollback可以回滚至未提交的命令  

#myisam 对于不支持事务的引擎，写一条操作就向磁盘写入 ,rollback则无法回滚上一条 
...
# 查询数据库
name = input("Name:")
# 确保sql语句正确   ----->name= "%s"要加双引号 不加双引号不对
